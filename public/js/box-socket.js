...
if (listButtonAddFriend.length > 0) {
    console.log(listButtonAddFriend)
    listButtonAddFriend.forEach(button => {
        button.addEventListener("click", () => {
            button.closest(".box-item").classList.add("add");
            const userId = button.getAttribute("btn-add-friend");
            socket.emit("CLIENT_ADD_FRIEND", userId);
        })
    })
}
// hếthết chức năng gửi yêu cầu kết bạn 
...
if (listButtonCanceldFriend.length > 0) {
    listButtonCanceldFriend.forEach(button => {
        button.addEventListener("click", () => {
            button.closest(".box-item").classList.remove("add")
...
const refuseFriend = (button) => {
    button.addEventListener("click", () => {
        button.closest(".box-item").classList.add("refuse")
...
if (listButtonRefusedFriend.length > 0) {
    listButtonRefusedFriend.forEach(button => {
        button.addEventListener("click", () => {
            ...
            button.closest(".box-item").classList.remove("add");
            button.closest(".box-item").classList.add("friend")
            const userId = button.getAttribute("btn-refuse-friend");
            console.log(userId)
            socket.emit("CLIENT_REFUSE_FRIEND_FRIEND", userId);
        })
    })
}
...
const acceptFriend = (button) => {
    button.addEventListener("click", () => {
        button.closest(".box-item").classList.add("accepted")
